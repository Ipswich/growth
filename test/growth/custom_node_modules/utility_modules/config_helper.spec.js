...
describe('config_helper.js tests: ', function() {
  ...
  describe('getConfig() tests', function() {
    ...
    it('should set and return a parsed JSON object', function() {
    ...
    it('should call fs.readFileSync() only once, returning the config from the initial call', function() {
      let fs_stub = sinon.stub(fs, 'readFileSync').returns(Buffer.alloc(0))
      sinon.stub(JSON, 'parse').returns(config_fixture)
      config_helper.getConfig()
      assert.deepEqual(config_helper.getConfig(), config_fixture)
      sinon.assert.calledOnce(fs_stub)
    })
  })

  describe('getWebData() tests', function() {
    beforeEach(function() {
      resetModules();
    });
    let config_fixture = {"test" : "test"}
    ...
    it('should call JSON.parse() only once, returning the config from the initial call', function() {
      let fs_stub = sinon.stub(fs, 'readFileSync').returns(Buffer.alloc(0))
      sinon.stub(JSON, 'parse').returns(config_fixture)
  ...
  describe('boardCheck() tests', function() {
    ...
    it('should throw an error', async function() {
      ...
      sinon.stub(config_helper, 'getBoardData').throws(new Error("Error!"))
      try {
        await config_helper.boardCheck(expected, true)
        assert.fail()
      } catch (e) {
        assert.typeOf(e, 'Error')
      }
  ...
  describe('nodeMailerCheck() tests', function() {
    ...
    it('should return an array of the passed object, "true", and "false"', async function() {
      ...
      let stub_input = sinon.stub(config_helper, 'getInput')
  ...
  describe('cameraCheck() tests', function(){
    ...
    it('should return an array of an object with true, and "true"', async function() {
      ...
      sinon.stub(config_helper, 'getInput').resolves('y')
      let actual = await config_helper.cameraCheck(test_config, false)
      expect(actual[0].camera_enable).to.be.true
      expect(actual[1]).to.be.true
    })
    it('should return an array of an object with false, and "true"', async function() {
      let test_config = {
      ...
      sinon.stub(config_helper, 'getInput').resolves('n')
      let actual = await config_helper.cameraCheck(test_config, false)
      expect(actual[0].camera_enable).to.be.false
      expect(actual[1]).to.be.true
    })
    it('should return an array of the passed object with "test", and "false"', async function() {
      let test_config = {
      ...
      let actual = await config_helper.cameraCheck(test_config, false)
      expect(actual[0].camera_enable).to.be.deep.equal('test')
      expect(actual[1]).to.be.false
    })
  })
  ...
  describe('configUpdate() tests', function() {
    it('should call writeFileSync', function() {
      ...
      config_helper.configUpdate(config)    
  ...
  describe('configChecker() tests', function() {
    it('should resolve true', async function() {
      ...
      sinon.stub(config_helper, 'cameraCheck').resolves([test_config, true])
    ...
    it('should resolve an error', async function() {
      ...
      await config_helper.configChecker(test_config, true).then(() => {
        assert.fail()
      }).catch((e) => {
        assert.typeOf(e, 'Error')
      })
