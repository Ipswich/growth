...
module.exports = class OutputState {
  ...
  addOutputSchedules(outputID, scheduleID) {
    for(let i = 0; i < this.data.length; i++){
      if(this.data[i].outputID == outputID){
        this.data[i].outputSchedules.push(scheduleID);
        console.log(this.data[i].outputSchedules)
        console.log("AFTER ADD: " + this.data[i].outputSchedules)
      }
    }
  }
  ...
  checkOutputSchedules(outputID, scheduleID) {
    for(let i = 0; i < this.data.length; i++){
      if(this.data[i].outputID == outputID){
        let data = this.data[i].outputSchedules.indexOf(scheduleID)
  ...
  removeOutputSchedules(outputID, scheduleID) {
    for(let i = 0; i < this.data.length; i++){
      if(this.data[i].outputID == outputID){
        console.log("BEFORE REMOVE: " + this.data[i].outputSchedules)
        let index = this.data[i].outputSchedules.indexOf(scheduleID);
        this.data[i].outputSchedules.splice(index, 1)
        console.log("AFTER REMOVE: " + this.data[i].outputSchedules)
      }
    }
  }
