...
function mapPins(outputs){
  var outputPin = startOutputPin;
  var pwmPin = startPWMPin
  for(i = 0; i < outputs.length; i++){
    if (outputPin > 53) {
      console.error("Mappings.js: getOutputMappings() failed, out of digital pins!");
      break;
    }
    outputs[i].OUTPUTPIN = outputPin;
    outputPin++;
    if (pwmPin > 13) {
      console.error("Mappings.js: getOutputMappings() failed, out of PWM pins!");
      break;
    }
    switch(outputs[i].outputType) {
      case 'Light':
        outputs[i].PWMPIN = pwmPin;
        pwmPin++;
        break;
    }
  };
