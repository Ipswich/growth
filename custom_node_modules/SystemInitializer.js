...
async function initializeOutputs(state) {
  config = config_helper.getConfig()
  return new Promise((resolve) => {
    ...
    if(config.relay_toggle_prevention){
      let relay_toggle_pin = mappings.getBoardMapping().OUTPUT_PINS[0]
    ...
    for(let i = 0; i < state.outputState.getOutputState().length; i++){
      ...
      try {
        ...
        printouts.errorPrintout("Arduino out of pins! Could not attach all outputs, please check your settings.")
      }
    }
...
async function doneEmitter_booted(DS18B20_Count, state, doneEmitter) {
  if(DS18B20_Count == 0 ){ 
    console.log("Initializing sensors. . .");
    await initializeSensors(state)
    console.log("Initializing outputs. . ."); 
...
async function doneEmitter_initialized(state){
  console.log("Initializing schedule. . .")
  await initializeSchedule(state)
  state.warnState = app.get('warnState')
  app.set('state', state)
