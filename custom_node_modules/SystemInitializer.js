...
module.exports.initialize = async function initialize(state, app) {
  ...
  return new Promise(async (resolve) => {
    ...
    for (let i = 0; i < state.sensorState.getSensorState().length; i++) {      
      if (state.sensorState.getSensorState()[i].sensorModel == "DS18B20") {
        DS18B20_Count++
...
async function initializeOutputs(state) {
  return new Promise((resolve) => {
    ...
    for(let i = 0; i < state.outputState.getOutputState().length; i++){
      let outputPin = state.outputState.getOutputState()[i].outputPin
      var output = new five.Relay({
        pin: outputPin,
        type: "NC"
      })
      state.outputState.getOutputState()[i].outputObject = output
      if(state.outputState.getOutputState()[i].outputPWMPin != null){
        let PWMPin = state.outputState.getOutputState()[i].outputPWMPin
        var PWM = new five.Led(PWMPin);
        state.outputState.getOutputState()[i].outputPWMObject = PWM
        //Handle board interrupts (turn off stuff)
        board.on('exit', () => {
          output.open()
          PWM.brightness(0)
  ...
  return new Promise((resolve) => {
    ...
    while (hardwareID > counter) {
      ...
      for(i; i < state.sensorState.getSensorState().length; i++){  
        let currentState = state.sensorState.getSensorState()[i]     
        let sensor;
