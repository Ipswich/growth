...
const takeImage = function() {
  ...
  let command = 'raspistill -o ' + output_file + flag_string
 ...
 */
const takeImage_between = function() {
  if (config.camera.active_hours.start == null || config.camera.active_hours.stop == null){
    module.exports.takeImage()
  } else {
    let start = moment(config.camera.active_hours.start, 'hh:mm')
    let stop = moment(config.camera.active_hours.stop, 'hh:mm')
    if(moment().isBetween(start, stop)){
      module.exports.takeImage()
    }
  }
}
module.exports.takeImage_between = takeImage_between