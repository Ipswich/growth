...
function filterOn(output, schedule, state) {
  if(output.outputController == "Manual"){
    ...
    if(schedule.scheduleType != "Manual"){
      ...
      state.outputState.setOutputScheduleState(output.outputID, "Output On", schedule.outputValue);
      // utils.debugPrintout("[" + output.outputName + "]" + "[Manual] ON @ " + schedule.outputValue + " - SKIPPED, SCHEDULE IN MANUAL STATE")
      return false;
    }
    if(output.lastOutputController == 'Schedule' && output.scheduleState == "Output On"){
      ...
      if(output.outputPWMObject){
        if(output.scheduleOutputValue == schedule.outputValue) {
          state.outputState.setOutputManualState(output.outputID, "Output On", schedule.outputValue);
          // utils.debugPrintout("[" + output.outputName + "]" + "[Manual] ON @ " + schedule.outputValue + " - SKIPPED, SAME AS PREVIOUS SCHEDULE STATE")
          return false;
        }
      } else {
        state.outputState.setOutputManualState(output.outputID, "Output On", schedule.outputValue);
    ...
    if(output.lastOutputController == 'Manual' && output.manualState == "Output On") {
      if(output.outputPWMObject){
        if(output.manualOutputValue == schedule.outputValue) {          
          // utils.debugPrintout("[" + output.outputName + "]" + "[Manual] ON @ " + schedule.outputValue + " - SKIPPED, SAME AS PREVIOUS MANUAL STATE")
          return false;
        }
      } else {
        // utils.debugPrintout("[" + output.outputName + "]" + "[Manual] ON @ " + schedule.outputValue + " - SKIPPED, SAME AS PREVIOUS MANUAL STATE")
    ...
    if(schedule.scheduleType == "Manual"){
      state.outputState.setOutputManualState(output.outputID, "Output On", schedule.outputValue);
      // utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] ON @ " + schedule.outputValue + " - SKIPPED, MANUAL IN SCHEDULE STATE")
      return false;
    }
    if(output.lastOutputController == 'Schedule' && output.scheduleState == "Output On") {
      if(output.outputPWMObject){
        if(output.scheduleOutputValue == schedule.outputValue) {          
          // utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] ON @ " + schedule.outputValue + " - SKIPPED, SAME AS PREVIOUS SCHEDULE STATE")
          return false;
        }
      } else {
        utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] ON @ " + schedule.outputValue + " - SKIPPED, SAME AS PREVIOUS SCHEDULE STATE")
    ...
    if(output.lastOutputController == 'Manual' && output.manualState == "Output On") {
      ...
      if(output.outputPWMObject){
        if(output.manualOutputValue == schedule.outputValue) {              
          state.outputState.setOutputScheduleState(output.outputID, "Output On", schedule.outputValue);
          // utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] ON @ " + schedule.outputValue + " - SKIPPED, SAME AS PREVIOUS MANUAL STATE")
          return false;
        }
      } else {
        state.outputState.setOutputScheduleState(output.outputID, "Output On", schedule.outputValue);
