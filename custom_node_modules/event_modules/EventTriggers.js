...
module.exports.triggerEvent = async function triggerEvent(schedule, state, event = null, triggerVal = null) {
  ...
  switch (switchCase) {
    case 'Output On':
      if(!filterOn(output, schedule, state, schedule.outputValue)) {
        return;
      } 
      // Turn on relay
    ...
    case 'Python Script':
      ...
      try {
        let python = spawn('python3', input_file);      
        utils.debugPrintout("Running python script: " + file)
        python.stdout.on('data', function (data) {
          utils.debugPrintout(data.toString())
        });
        python.stderr.on('data', function (data) {
          utils.debugPrintout("ERROR: " + data.toString())
        })
        python.on('close', (code) => {
          if (code == 3){
            utils.debugPrintout(file + " exited, code: " + code + " - reading script output file, " + fileName + ".txt" + ".")          
            // Read script output file
            fs.readFile(script_directory + fileName + ".txt", "utf8", (err, data) => {
              if(err){
                utils.debugPrintout("Error reading output file: " + err)
              } else {
                let parsed_data = {}
                ...
                try {
                  ...
                  if (parsed_data.output == 1){
                    ...
                    if(!filterOn(output, schedule, state, parsed_data.outputPWM)) {
                      return;
                    } 
                    // Turn on relay
                    if(output.outputPWMObject){
                      ...
                      //If inversion set, use 255 - PWM value
                      if (output.outputPWMInversion == 0) {
                        value = Math.round(base * parsed_data.outputPWM);
                      } else {
                        value = 255 - Math.round(base * parsed_data.outputPWM);
                      }
                      // Adjust PWM value and turn on relay
                      output.outputPWMObject.brightness(value);        
                      output.outputObject.close();
                      utils.debugPrintout(output.outputName + ": [" + output.outputController + "] ON @ " + parsed_data.outputPWM + "% - [Output Pin: " + output.outputPin + ", PWM Pin: " + output.outputPWMPin + "]");      
                    } else {
                      utils.debugPrintout(output.outputName + ": [" + output.outputController + "] ON - [Output Pin: " + output.outputPin + "]");      
                      output.outputObject.close();
                    }
                    state.outputState.setOutputScheduleState(output.outputID, "Output On", parsed_data.outputPWM);                  
                  }
                  ...
                  if (parsed_data.output == 0){
                    ...
                    output.outputObject.open();
                    if(schedule.scheduleType == 'Manual'){
                      state.outputState.setOutputManualState(output.outputID, "Output Off", parsed_data.outputPWM);
                    } else {
                      state.outputState.setOutputScheduleState(output.outputID, "Output Off", parsed_data.outputPWM);
                    }
                  }
                } catch (e) {
                  utils.debugPrintout("Error reading output file: " + e)
                }
              }
            })
          } else {
        ...
        utils.debugPrintout("Error opening python file: " + e)
      }
      break;
  }
}
...
function filterOn(output, schedule, state, outputValue) {
  if(output.outputController == "Manual"){
    ...
    if(schedule.scheduleType != "Manual"){
      ...
      utils.debugPrintout("[" + output.outputName + "]" + "[Manual] ON @ " + outputValue + "% - SKIPPED, SCHEDULE IN MANUAL STATE")
      return false;
    }
    if(output.lastOutputController == 'Schedule' && output.scheduleState == "Output On"){
      ...
      if(output.outputPWMObject){
        if(output.scheduleOutputValue == outputValue) {
          ...
          utils.debugPrintout("[" + output.outputName + "]" + "[Manual] ON @ " + outputValue + "% - SKIPPED, SAME AS PREVIOUS SCHEDULE STATE")
          return false;
        }
      } else {
        state.outputState.setOutputManualState(output.outputID, "Output On", outputValue);
    ...
    if(output.lastOutputController == 'Manual' && output.manualState == "Output On") {
      if(output.outputPWMObject){
        if(output.manualOutputValue == outputValue) {          
          utils.debugPrintout("[" + output.outputName + "]" + "[Manual] ON @ " + outputValue + "% - SKIPPED, SAME AS PREVIOUS MANUAL STATE")
          return false;
        }
      } else {
        utils.debugPrintout("[" + output.outputName + "]" + "[Manual] ON @ " + outputValue + "% - SKIPPED, SAME AS PREVIOUS MANUAL STATE")
    ...
    if(schedule.scheduleType == "Manual"){
      state.outputState.setOutputManualState(output.outputID, "Output On", outputValue);
      utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] ON @ " + outputValue + "% - SKIPPED, MANUAL IN SCHEDULE STATE")
      return false;
    }
    if(output.lastOutputController == 'Schedule' && output.scheduleState == "Output On") {
      if(output.outputPWMObject){
        if(output.scheduleOutputValue == outputValue) {          
          utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] ON @ " + outputValue + "% - SKIPPED, SAME AS PREVIOUS SCHEDULE STATE")
          return false;
        }
      } else {
        utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] ON @ " + outputValue + "% - SKIPPED, SAME AS PREVIOUS SCHEDULE STATE")
    ...
    if(output.lastOutputController == 'Manual' && output.manualState == "Output On") {
      ...
      if(output.outputPWMObject){
        if(output.manualOutputValue == outputValue) {              
          state.outputState.setOutputScheduleState(output.outputID, "Output On", outputValue);
          utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] ON @ " + outputValue + "% - SKIPPED, SAME AS PREVIOUS MANUAL STATE")
          return false;
        }
      } else {
        state.outputState.setOutputScheduleState(output.outputID, "Output On", outputValue);
...
function filterOff(output, schedule, state) {
  ...
  if(output.outputController == "Manual"){
    if(schedule.scheduleType != "Manual"){          
      ...
      utils.debugPrintout("[" + output.outputName + "]" + "[Manual] OFF - SKIPPED, SCHEDULE IN MANUAL STATE")
      return false;
    } 
    ...
    if(output.lastOutputController == 'Schedule' && output.scheduleState == "Output Off") {
      ...
      utils.debugPrintout("[" + output.outputName + "]" + "[Manual] OFF - SKIPPED, SAME AS PREVIOUS SCHEDULE STATE")
      return false;
    }
    if(output.lastOutputController == 'Manual' && output.manualState == "Output Off"){
      utils.debugPrintout("[" + output.outputName + "]" + "[Manual] OFF - SKIPPED, SAME AS PREVIOUS MANUAL STATE")
      return false;
    }     
  // Else we're in schedule control
    ...
    if(schedule.scheduleType == "Manual"){          
      state.outputState.setOutputManualState(output.outputID, "Output Off");
      utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] OFF - SKIPPED, MANUAL IN SCHEDULE STATE")
      return false;
    }
    //if old state was set to off, return        
    if(output.lastOutputController == 'Schedule' && output.scheduleState == "Output Off") {
      utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] OFF - SKIPPED, SAME AS PREVIOUS SCHEDULE STATE")
      return false;
    }
    //Even if manual state is off, this should still be triggered.
    // if(output.lastOutputController == 'Manual' && output.manualState == "Output Off"){
    //   state.outputState.setOutputScheduleState(output.outputID, "Output Off");
    //   utils.debugPrintout("[" + output.outputName + "]" + "[Schedule] OFF - SKIPPED, SAME AS PREVIOUS MANUAL STATE")
    //   return false;
    // } 
  }
  return true;
