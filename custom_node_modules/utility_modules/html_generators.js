...
async function regenerateChartData() {
    ...
    for (let key in INTERVALS) {
        ...
        if (chartData[interval] === undefined) {
            ...
            chartData[interval] = await updateChartData(interval)
        }
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let key in previousSensorData) {
                ...
                while (nowMinusInterval.isAfter(new moment(previousSensorData[key][0].x))) {
                    console.log(`Yeeting oldest data in ${key}: ${previousSensorData[key][0].x}`);
