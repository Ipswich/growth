...
module.exports.getBoardMapping = function getBoardMapping(){
  if(board == null) {
    ...
    const board_mappings = JSON.parse(fs.readFileSync(config_helper.BOARD_MAPPINGS_LOCATION).toString())    
...
function mapOutputPins(outputs) {
  ...
  if(config.relay_toggle_prevention){
    try {
      pinCountCheck(outputPins, "output")
    } catch (e) {
      throw e
    }
    outputPins.shift();
  ...
  for(i = 0; i < sensors.length; i++) {
    ...
    switch (sensors[i].sensorProtocol) {
      ...
      case "ANALOG":
        ...
        try {
          pinCountCheck(analogPins, "analog")
        } catch (e) {
          throw e
        }
      ...
      case "ONEWIRE":
        if (onewirePin == -1) {
          ...
          try {
            pinCountCheck(analogPins, "sensor")
          } catch (e) {
            throw e
          }
