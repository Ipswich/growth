...
function mapOutputPins(outputs) {
  ...
  for(i = 0; i < outputs.length; i++){
    ...
    if (outputPin > pinout.END_OUTPUT_PIN) {
      utils.errorPrintout("Mappings.js: mapOutputPins() failed, out of digital pins!");
      process.exit(-1)
    }
    ...
    if (pwmPin > pinout.END_PWM_PIN) {
      utils.errorPrintout("Mappings.js: mapOutputPins() failed, out of PWM pins!");
      process.exit(-1)
    }
...
function mapSensorPins(sensors) {
  ...
  for(i = 0; i < sensors.length; i++) {
    ...
    if (sensorPin > pinout.END_SENSOR_PIN || analogPin > pinout.END_ANALOG_PIN) {
      utils.errorPrintout("Mappings.js: mapSensorPins() failed, out of pins!");
      process.exit(-1)
      break;
    }
