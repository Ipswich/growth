...
module.exports.createTestDBStructure = async function(){
  return new Promise((resolve, reject) => {
    let query
    if(process.env.TEST_RUNNER == 'github'){
      if(config.database.host = 'localhost'){
        config.database.host = '127.0.0.1'
      }
      query = 'mysql -h ' + config.database.host + ' -P ' + config.database.port + ' -u root -p' + config.database.password + ' growth-test < ./SQL/schema_testDB.sql'
    } else {
      query = 'mysql -u ' + config.database.user + ' -p' + config.database.password + ' growth-test < ./SQL/schema_testDB.sql'
    }
    dir = exec(query, function(err, stdout, stderr) {
      if (err) {
        reject(err)
      }
      console.log(stdout);
...
module.exports.addTestDBData = async function(){
  return new Promise((resolve, reject) => {
    let query
    if(process.env.TEST_RUNNER == 'github'){
      if(config.database.host = 'localhost'){
      ...
      query = 'mysql -h ' + config.database.host + ' -P ' + config.database.port + ' -u root -p' + config.database.password + ' growth-test < ./SQL/testData.sql'
    } else {
      query = 'mysql -u ' + config.database.user + ' -p' + config.database.password + ' growth-test < ./SQL/testData.sql'
    }
    dir = exec(query, function(err, stdout, stderr) {
      if (err) {
        reject(err)
