...
module.exports.createTestDBStructure = async function(){
  return new Promise((resolve, reject) => {
    ...
    // console.log(query)
    dir = exec('mysql  -h ' + config.database.host + ' -P ' + config.database.port + ' -u ' + config.database.user + ' -p' + config.database.password+ ' growth-test < ./SQL/schema_testDB.sql', function(err, stdout, stderr) {
      if (err) {
        reject(err)
      }
      console.log(stdout);
