...
module.exports.addNewOutput = async function(type, name, description) {
  return new Promise((resolve, reject) => {
    let query = 'CALL addNewOutput(' + type + ',' + name + ',' + description + ')'
    pool.query(query, (error, results, fields) => {
      //Error on problem
      if(error) {
...
module.exports.addNewOutputType = async function(type, enabled) {
  return new Promise((resolve, reject) => {
    let query = 'CALL addNewOutputType(' + type + ',' + enabled + ')'
    pool.query(query, (error, results, fields) => {
      //Error on problem
      if(error) {
...
module.exports.addNewSchedule = async function(scheduleType, eventID, sensorID, sensorValue, outputID, outputValue, scheduleComparator, eventTriggerTime, eventDuration, eventInterval, scheduleStartDate, scheduleStopDate, enabled, addedBy, disabledBy) {
  return new Promise((resolve, reject) => {
    let query = 'CALL addNewSchedule(' + scheduleType + ',' + eventID + ',' + sensorID + ',' + sensorValue + ',' + outputID + ',' + outputValue + ',' + scheduleComparator + ',' + eventTriggerTime + ',' + eventDuration + ',' + eventInterval + ',' + scheduleStartDate + ',' + scheduleStopDate + ',' + enabled + ',' + addedBy + ',' + disabledBy + ')'    
    pool.query(query, (error, results, fields) => {
      //Error on problem
      console
      if(error) {
...
module.exports.addNewSensor = async function(model, type, location, units, hardwareID, sensorProtocol) {
  return new Promise((resolve, reject) => {
    let query = 'CALL addNewSensor(' + model + ',' + type + ',' + location + ',' + units + ',' + hardwareID + ',' + sensorProtocol + ')'
    pool.query(query, (error, results, fields) => {
      //Error on problem
      if(error) {
...
module.exports.addNewUser = async function(username, hash) {
  return new Promise((resolve, reject) => {
    let query = 'CALL addNewUser(' + username + ',' + hash + ')'
    pool.query(query, (error, results, fields) => {
      //Error on problem
      if(error) {
