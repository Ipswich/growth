...
module.exports.CONFIG_LOCATION = './config/config.json'
module.exports.DEFAULT_CONFIG_LOCATION = './config/default_config.json'
module.exports.BOARD_MAPPINGS_LOCATION = './config/board_mappings.json'
module.exports.WEB_DATA_LOCATION = './config/web-data-config.json'
module.exports.DEAFULT_WEB_DATA_LOCATION = './config/web-data-config-default.json'
 ...
 * returns it. Otherwise returns the previously loaded config file as a copy
 * (read-only).
 * @param {string} config_location File location for the configuration file to load.
 * @returns the JSON version of the passed file.
 */
const getConfig = function(config_location = exports.CONFIG_LOCATION) {
...
let web_data = undefined;
// Stubbable, easier to test than mocking require and means no dependencies.
/**
 * Singleton; Reads web_data from file - if web_data is undefined, reads it and
 * returns it. Otherwise returns the previously loaded web_data file as a copy
 * (read-only).
 * @param {string} location File location for the file to load.
 * @returns the JSON version of the passed file.
 */
const getWebData = function(location = exports.WEB_DATA_LOCATION) {
  if (web_data) return JSON.parse(JSON.stringify(web_data))
  web_data = JSON.parse(fs.readFileSync(location).toString())
  return JSON.parse(JSON.stringify(web_data))
}
...
const initializeConfig = async function() {
  ...
  return new Promise(async (resolve, reject) => {
    if(fs.existsSync(exports.CONFIG_LOCATION)){
      config = JSON.parse(fs.readFileSync(exports.CONFIG_LOCATION).toString())
      resolve(config)
    } else {
      simpleLogPrintout("[SETUP] Could not locate config.json, copying from default_config.json.");
      try {
        fs.copyFileSync(exports.DEFAULT_CONFIG_LOCATION, exports.CONFIG_LOCATION)
        config = JSON.parse(fs.readFileSync(exports.CONFIG_LOCATION).toString())
        resolve(config)
      } catch(err) {
...
const initializeWebData = async function() {
  let web_data;
  return new Promise(async (resolve, reject) => {
    if(fs.existsSync(exports.WEB_DATA_LOCATION)){
      web_data = JSON.parse(fs.readFileSync(exports.WEB_DATA_LOCATION).toString())
      resolve(web_data)
    } else {
      simpleLogPrintout("[SETUP] Could not locate web-data-config.json, copying from web-data-config-default.json.");
      try {
        fs.copyFileSync(exports.DEAFULT_WEB_DATA_LOCATION, exports.WEB_DATA_LOCATION)
        web_data = JSON.parse(fs.readFileSync(exports.WEB_DATA_LOCATION).toString())
        resolve(web_data)
      } catch(err) {
...
const getBoardData = function() {
  ...
  let board_list = ''
  try {
    board_list_JSON = JSON.parse(fs.readFileSync(exports.BOARD_MAPPINGS_LOCATION).toString())
  } catch (e) {
 ...
 */
const configUpdate = function(config) {
  try {      
    fs.writeFileSync(exports.CONFIG_LOCATION, JSON.stringify(config))
 ...
 *  [config, update]
 */
const cameraCheck = async function(config, update) {
  if (config.camera_enable == 'null'){
    simpleLogPrintout("\n[SETUP] Camera enable has not been changed in config.")
    input = await exports.getInput("Would you like to enable this? (y/N)")
    if (input == 'y' || input == 'Y'){
      config.camera_enable = true
    } else {
      config.camera_enable = false
    }
    update = true
  }
  return [config, update]
}
module.exports.cameraCheck = cameraCheck
...
const configChecker = async function(config){
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      [config, update] = await exports.cameraCheck(config, update);
      if(update == true){
        exports.configUpdate(config)
      }
      resolve(warnState)
    } catch (e) {
      reject(e)
