...
async function getSchedule(type) {
  let schedule = await dbcalls.getEnabledLiveSchedules()
  let result = [];
...
/**
 * Gets the name of the latest image.
 * @returns The name of the most recent image in the configuration file's image
 * directory
 */
async function getLatestImageName() {
  let config = config_helper.getConfig()
  try {
    const files = await fsPromises.readdir(config.camera.image_directory);
    if (files.length == 0){
    ...
    for (const file of files) {
      let stat = await fsPromises.stat(config.camera.image_directory + '/' + file)      
...
module.exports.getLatestImageName = getLatestImageName
