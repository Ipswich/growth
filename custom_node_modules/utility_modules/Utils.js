...
module.exports.getSensorVal = async function getSensorVal(state, i) {
  let obj = state.sensorState.data[i].sensorObject;
  let units = state.sensorState.data[i].sensorUnits;
  let data = {}
  data.sensorType = state.sensorState.data[i].sensorType
  data.sensorLocation = state.sensorState.data[i].sensorLocation
  data.sensorID = state.sensorState.data[i].sensorID;
  return new Promise((resolve, reject) => {
    if(obj != null) {
      obj.once('data', () => {
        parseSensorValue(i, state, units, data, obj)
        resolve(data);
      })
      if(process.env.NODE_ENV == 'test'){
        obj.emit('data')
...
function parseSensorValue(i, state, units, data, obj){
  switch (state.sensorState.data[i].sensorType) {
    case 'Temperature':
      if (units == "°C" || units == "C") {
        data.val = obj.C;
...
module.exports.parseSensorValue = parseSensorValue
...
module.exports.getIndexData = async function getIndexData(res, req, interval = CHART_INTERVAL){
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      //Get python scripts
      var python = {python: getScriptFileNames('py')}
...
module.exports.getAndFormatChartData = getAndFormatChartData
...
function getScriptFileNames(suffix) {
  let files = fs.readdirSync('./scripts')
  let sortedFiles = files.filter(e => e.split('.').pop() == suffix);
  return sortedFiles;
}
