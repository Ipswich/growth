...
module.exports.getIndexData = async function getIndexData(res, req, interval = CHART_INTERVAL){
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      var sensorData = await getChartDataByInteval(interval)            
      console.log("ELAPSED [CHART DATA]: " + moment().diff(start) + " ms")
      start = moment()
      var sensorTypes = {sensorTypes: await dbcalls.getEnabledSensorTypes()};
      console.log("ELAPSED [SENSOR TYPES]: " + moment().diff(start) + " ms")
      start = moment()
      var scheduleData = {scheduleData: await dbcalls.getEnabledLiveSchedules()};
      console.log("ELAPSED [SCHEDULE DATA]: " + moment().diff(start) + " ms")
      start = moment()
      ...
      console.log("ELAPSED [OUTPUTS]: " + moment().diff(start) + " ms")
      start = moment()
      //Get enabled events
      var events = {events: await dbcalls.getEnabledEvents()};
      console.log("ELAPSED [EVENTS]: " + moment().diff(start) + " ms")
      start = moment()
      //Get enabled sensors
      var sensors = {sensors: await dbcalls.getEnabledSensors()};
      console.log("ELAPSED [SENSORS]: " + moment().diff(start) + " ms")      
      start = moment()
      ...
      var data = Object.assign({}, web_data, sensorTypes, sensorData, scheduleData, outputs, events, sensors, intervals, state, authenticated);
      ...
      console.log("RESOLVED: " + moment().diff(start) + " ms")    
      resolve(packet);
    } catch (e) {
      //If error, error
      reject(e);
...
async function getAndFormatChartData(interval) {
 return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      console.log("ELAPSED [QUERY]: " + moment().diff(start) + " ms")
      start = moment()
      ...
      console.log("ELAPSED [FORMAT LOGTIME]: " + moment().diff(start) + " ms")
      start = moment()
      ...
      console.log("ELAPSED [FORMAT DATA]: " + moment().diff(start) + " ms")      
    } catch(e) {
      reject(e)
    }
    resolve(sensorData)
...
function getChartDataByInteval(interval){
  return chartData[interval]
}

module.exports.regenerateChartData = async function regenerateChartData(){  
  let start = moment()
  let total = 0;
  for (let key in INTERVALS){
    let interval = INTERVALS[key].interval
    chartData[interval] = await getAndFormatChartData(interval)
    total += moment().diff(start)
    console.log("ELAPSED ["+INTERVALS[key].title+"]: " + moment().diff(start) + " ms\n")
    start = moment()
  }
