 ...
 * @returns 
 */
function initializeOutputs(state) {
  ...
  relayToggleCheck(state, config)

  //Ensure that max listeners for events is appropriate to suppress warnings.
  state.board.setMaxListeners(state.board.getMaxListeners() + maxListenerCheck(state, config))  
 ...
 * @param {object} board 
 */
function relayToggleCheck(state, config){
  if(config.relay_toggle_prevention){ 
